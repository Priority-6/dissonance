...
//        int claimColor = currentClaim.type() == ClaimType.CHUNK_LOADED ? 0xFFFF9900 : currentClaim.color();
    ...
    public Component getChunkHighlightBluntTooltip(ResourceKey<Level> dimension, int chunkX, int chunkZ) {
        if (!WorldMap.settings.displayClaims)
            return null;
        if (!ClientClaims.CLAIMS.containsKey(dimension))
            return null;
...
//                                :
    ...
    public int calculateRegionHash(ResourceKey<Level> dimension, int regionX, int regionZ) {
        ...
        if (!ClientClaims.CLAIMS.containsKey(dimension))
            return 0;
        if (!regionHasHighlights(dimension, regionX, regionZ))
            return 0;
        ...
        for (int regionLocalX = 0; regionLocalX < 32; ++regionLocalX) {
            accumulator = this.accountClaim(accumulator,
                    topRegionClaimed ? this.manager.get(dimension, topChunkX + regionLocalX, topChunkZ + 31) : null);
            accumulator = this.accountClaim(accumulator,
                    rightRegionClaimed ? this.manager.get(dimension, rightChunkX, rightChunkZ + regionLocalX) : null);
            accumulator = this.accountClaim(accumulator,
                    bottomRegionClaimed ? this.manager.get(dimension, bottomChunkX + regionLocalX, bottomChunkZ)
                            : null);
            accumulator = this.accountClaim(accumulator,
                    leftRegionClaimed ? this.manager.get(dimension, leftChunkX + 31, leftChunkZ + regionLocalX) : null);
