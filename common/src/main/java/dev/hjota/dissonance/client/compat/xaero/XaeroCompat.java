...
public class XaeroCompat {
    ...
    public void addRightClickOptions(GuiMap screen, ArrayList<RightClickOption> options,
            MapTileSelection mapTileSelection, MapProcessor mapProcessor) {
        if (mapTileSelection != null) {
            if (mapProcessor.getMapWorld().isUsingCustomDimension()) {
                options.add(new RightClickOption("gui.xaero_cadmus_claim_selection_out_of_dimension", options.size(),
                        screen) {
            ...
            if (hasUnclaimed) {
                int _right = right;
                int _bottom = bottom;
                options.add(new RightClickOption("gui.dissonance.claim_chunks", options.size(), screen) {
                    @Override
                    public void onAction(Screen screen) {
            ...
            if (hasClaimed) {
                int _right = right;
                int _bottom = bottom;
                options.add(new RightClickOption("gui.dissonance.unclaim_chunks", options.size(), screen) {
                    @Override
                    public void onAction(Screen screen) {
            ...
            if (hasUnclaimPermission) {
                ...
                options.add(new RightClickOption("gui.dissonance.admin_unclaim_chunks", options.size(), screen) {
                    @Override
                    public void onAction(Screen screen) {
                        manager.changeSelection(dimension, left, top, _right, _bottom, false, null,
                                _hasUnclaimPermission);
                    }
                });
            }
            if (hasUnchunkloaded) {
                int _right = right;
                int _bottom = bottom;
                options.add(new RightClickOption("gui.dissonance.chunkload_chunks", options.size(), screen) {
                    @Override
                    public void onAction(Screen screen) {
            ...
            if (hasChunkloaded) {
                int _right = right;
                int _bottom = bottom;
                options.add(new RightClickOption("gui.dissonance.unchunkload_chunks", options.size(), screen) {
                    @Override
                    public void onAction(Screen screen) {
