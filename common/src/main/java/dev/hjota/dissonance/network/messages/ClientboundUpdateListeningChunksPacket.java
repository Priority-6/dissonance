...
public record ClientboundUpdateListeningChunksPacket(
        ...
        String teamId) implements Packet<ClientboundUpdateListeningChunksPacket> {

    public static final ResourceLocation ID = new ResourceLocation(Dissonance.MOD_ID, "update_listening_chunks");
    ...
    private static class Handler extends CodecPacketHandler<ClientboundUpdateListeningChunksPacket> {
        private static final MapCodec<ChunkPos, Pair<Boolean, ClaimType>> CHUNK_POS_CLAIM_CODEC = new MapCodec<>(
                ExtraByteCodecs.CHUNK_POS, ObjectByteCodec.create(ByteCodec.BOOLEAN.fieldOf(Pair::getFirst),
                        ClaimType.CODEC.fieldOf(Pair::getSecond), Pair::new));
        ...
        public PacketContext handle(ClientboundUpdateListeningChunksPacket message) {
            return (player, level) -> ClientClaims.get(message.dimension).update(message.id(), message.displayName(),
                    message.color(), message.claims(), message.teamId());
        }
    }
}
